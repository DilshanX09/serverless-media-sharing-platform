...
import { cacheGetJson, cacheSetJson, memoryCacheSet, memoryCacheGet } from "@/lib/cache";
import { cacheKeys } from "@/lib/cacheKeys";
import { withCacheHeaders, cacheHeaders } from "@/lib/cacheHeaders";

// Request deduplication cache - pending requests
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Check Redis cache
  const cached = await cacheGetJson<typeof memCached>(key);
  if (cached) {
    // Populate memory cache
    memoryCacheSet(key, cached, 25);
    return withCacheHeaders(
  ...
  const fetchFeed = async () => {
    ...
    // Cache for 45 seconds (increased from 20s)
    await cacheSetJson(key, payload, 45);
    memoryCacheSet(key, payload, 40);

    const response = NextResponse.json(payload, { status: 200 });
    return withCacheHeaders(response, cacheHeaders.feed);
  };

  const feedPromise = fetchFeed().finally(() => {
    pendingRequests.delete(key);
  });

  pendingRequests.set(key, feedPromise);
  return feedPromise;
}
